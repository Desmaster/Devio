		...
		for (int x = 0; x < WORLD_WIDTH; x++) {
			for (int y = 0; y < WORLD_HEIGHT; y++) {
				if (x >= 0 && x < 10 || x >= 11 && x < Display.getWidth() - 1) {
					blocks[x][y] = new Block(textures[random.nextInt(3)], x * BLOCK_SIZE,
							y * BLOCK_SIZE);
				} else {
					blocks[x][y] = new Block(textures[ID_SAND], x * BLOCK_SIZE,
							y * BLOCK_SIZE);
				}
			}
		}
	...
	public Block[][] generateWorld(int width, int height) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				blocks[x][y] = new Block(textures[ID_GRASS], x * BLOCK_SIZE, y
						* BLOCK_SIZE);
			}
		}
		return blocks;
